...
# 用法：scripts/verify-integration.sh [web_base]   默认 http://127.0.0.1:5176
set -uo pipefail

BASE="${1:-http://127.0.0.1:5176}"
CURL="curl -sS -m 8"
fail=0
...
runs=$($CURL "$BASE/api/v1/dev-demo/workspace" "${AUTH[@]}" | python3 -c 'import json,sys; print(len(json.load(sys.stdin)["runs"]))' 2>/dev/null)
[ -n "${runs:-}" ] && [ "$runs" -gt 0 ] 2>/dev/null && echo "PASS 运行中心 · workspace ($runs)" || { echo "FAIL 运行中心"; fail=1; }
